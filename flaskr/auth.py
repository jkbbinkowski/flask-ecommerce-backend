...
def login():
    if flask.request.method == 'GET':
        ...
        if len(auth_data_db) == 0 or (not werkzeug.security.check_password_hash(auth_data_db[0]['passHash'], data['log-pass'])):
            return {'errors': flask.g.errors['auth']['invalid_email_or_password']}, 404
        ...
        return flask.g.successes['auth']['logged-in'], 200
...
def register():
    if flask.request.method == 'GET':
        ...
        if len(emails) != 0:
            return {'errors': flask.g.errors['auth']['email_already_exists']}, 400
        ...
        return flask.g.successes['auth']['registered'], 201
...
def forgot_password():
    if flask.request.method == 'GET':
        ...
        try:
            ...
            return {'errors': flask.g.errors['auth']['forgot-pass-token-already-generated']}, 409
        ...
        return flask.g.successes['auth']['forgot-pass-token-generated'], 200
...
def new_forgot_password(token):
    if flask.request.method == 'GET':
        ...
        return flask.g.successes['auth']['new-password-set'], 200
...
def validate_login_data(data):
    ...
    if ('@' not in data['log-email']) or ('.' not in data['log-email']):
        errors.append(flask.g.errors['auth']['invalid_email'])
...
def validate_register_data(data):
    ...
    if (data['reg-fn'] == '') or (len(data['reg-fn']) > 45):
        errors.append(flask.g.errors['auth']['invalid_name'])
    if (data['reg-ln'] == '') or (len(data['reg-ln']) > 255):
        errors.append(flask.g.errors['auth']['invalid_last_name'])
    if ('@' not in data['reg-email']) or ('.' not in data['reg-email']):
        errors.append(flask.g.errors['auth']['invalid_email'])
    if not re.search(r'[a-z]', data['reg-pass']):
        errors.append(flask.g.errors['auth']['invalid_pass_lower_case'])
    if not re.search(r'[A-Z]', data['reg-pass']):
        errors.append(flask.g.errors['auth']['invalid_pass_upper_case'])
    if not re.search(r'[0-9]', data['reg-pass']):
        errors.append(flask.g.errors['auth']['invalid_pass_digit'])
    if not re.search(r'[0-9]', data['reg-ph']) or (len(data['reg-ph']) > 20):
        errors.append(flask.g.errors['auth']['invalid_phone'])
    if len(data['reg-pass']) < int(config['AUTH']['min_password_length']):
        errors.append(flask.g.errors['auth']['invalid_pass_length'])
